...
function ipvs() {
    ...
    for node_ip in ${NODE_IPS[@]}; do
        echo ">> ${node_ip}"
        ssh root@${node_ip} "yum -y install ipvsadm  ipset"
        ssh root@${node_ip} "modprobe -- ip_vs && modprobe -- ip_vs_rr && modprobe -- ip_vs_wrr && modprobe -- ip_vs_sh && modprobe -- nf_conntrack_ipv4"
        ssh root@${node_ip} "cat > /etc/sysconfig/modules/ipvs.modules <<EOF
modprobe -- ip_vs
modprobe -- ip_vs_rr
modprobe -- ip_vs_wrr
modprobe -- ip_vs_sh
